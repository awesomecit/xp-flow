...
## Prossimo sprint (2 giorni) — azioni dalla retro #3 (docs/retro/2026-34.md)
- [ ] R1 — Primo ciclo formale completo: planning appende `sprint/avviato`, `/chiusura` appende `sprint/chiuso` (assorbe tech debt #1 e #4); `xpflow status` riconosce apertura/chiusura — 2 SP
- [ ] R2 — Sblocco catena PR (ex A2, decisione repo pubblico): scan contenuti → flip a public (gate umano) → branch protection + required checks + auto-merge + `PR_REVIEW_ENABLED`/secret — 2 SP
...
- [ ] Metodo — app template baseline della fabbrica in `docs/drafts/2026-08-17-xpflow-app-template-baseline.md` — VALUTATA in retro #3 (17/08): resta in drafts, ratifica al trigger "prima app di prodotto nuova" (notifiche o home-gateway), insieme alla Fase 0 del report security/observability
